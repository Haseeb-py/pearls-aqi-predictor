...
def get_aqi_history(city: str, hours: int = 72, offset_hours: int | None = None) -> dict[str, Any]:
    ...
    frame = _city_data(city).tail(max(1, min(required_hours, 720)))
    ...
    if not points:
        raise ValueError("No historical AQI observations are available.")
    result: dict[str, Any] = {"city": city, "hours": len(points), "is_stale": _stale(points[-1]["time"]), "points": points}
    if offset_hours is not None:
        latest_time = pd.Timestamp(points[-1]["time"])
        target_time = latest_time - pd.Timedelta(hours=offset_hours)
...
def _fallback_crisis_assessment(text: str, history: list[str]) -> bool:
    ...
    patterns = (
        ...
        r"\beveryone would be better off without me\b", r"\beveryone'?s better off without me\b",
    ...
    continuation_phrases = (
        "i still feel that way", "still feel that way", "i still mean it", "i meant it",
        "yes i might", "yes, i might", "i might do it", "i am thinking about doing it",
        "i'm thinking about doing it", "still thinking about it",
    )
    if any(phrase in text for phrase in continuation_phrases):
        context = " ".join(history[-3:]).lower()
        return any(term in context for term in (
            "better off without me", "want to die", "end my life", "hurt myself",
            "harm myself", "kill myself", "suicide", "self-harm", "not worth living",
        ))

    if re.search(r"\b(just )?want (it|this) to end\b", text):
        context = " ".join(history[-3:]).lower()
        return any(term in context for term in (
            "alone", "hopeless", "better off without", "not alive", "hurt myself", "suicide", "giving up",
        ))
    return False
...
def _groq_grounded_response(
    ...
    lower = message.lower()
    requested_style = "eli5" if any(phrase in lower for phrase in (
        "like i'm five", "like im five", "like i am five", "eli5", "five year old", "5 year old", "very simple", "super simple",
    )) else "normal"
    payload = {
        "message": message,
        "route": route,
    ...
    system = (
        "You are Pearls AQI Copilot. Answer the CURRENT user request naturally and directly using ONLY tool_evidence and the supplied "
        "US AQI reference. Never invent measurements, forecasts, historical values, dates, weather values, sources, cities, trends, "
        "comparisons, or causal claims. Answer only the requested scope: for current_aqi do not volunteer forecasts or 24h change; for "
        "forecast discuss only future AQI; for current_forecast present both; for history use requested_point when present; for comparison "
        "with 'right now/current/currently' compare current values, not tomorrow. Say 'among the supported cities' for cleanest-city claims. "
        "If requested_style=eli5, speak as if to a five-year-old: use very short sentences, everyday words, one simple analogy, and avoid "
...
def _has_aqi_intent(message: str, history: list[str]) -> bool:
    ...
    direct_terms = (
        ...
        "history", "historical", "last week", "past month", "last few days", "yesterday", "day ago", "days ago", "aqi trend",
    ...
    if "compare" in text and supported_mentions >= 2:
        return True
    if any(term in text for term in ("all six cities", "all cities", "every city")) and any(term in text for term in ("best", "worst", "cleanest", "healthiest", "safest")):
        return True
    activity_question = any(term in text for term in (
        "jog", "run", "walk", "exercise", "workout", "outdoor", "outdoors", "outside",
        "picnic", "play", "kids", "children", "elderly", "older adult", "commute", "commuting", "school", "bike", "football",
    ))
...
def _history_hours(text: str) -> int:
    ...
    if any(term in text for term in ("week", "7 day")):
        return 168
    if any(term in text for term in ("few days", "3 day", "past 3", "three days", "two days", "2 days")):
        return 72
    if any(term in text for term in ("yesterday", "1 day", "one day")):
        return 24
    return 72


def _history_offset_hours(text: str) -> int | None:
    if any(term in text for term in ("yesterday", "one day ago", "1 day ago")):
        return 24
    if any(term in text for term in ("two days ago", "2 days ago")):
        return 48
    if any(term in text for term in ("three days ago", "3 days ago")):
        return 72
    return None
...
def _answer(message: str, evidence: dict[str, Any], cities: list[str]) -> str:
    ...
    if "comparison" in evidence:
        ...
        current_rank = sorted((entry["current"]["aqi"], entry["city"]) for entry in rows)
        tomorrow_rank = sorted((entry["forecast"]["forecasts"][0]["aqi"], entry["city"]) for entry in rows)
        future_requested = any(term in text for term in ("tomorrow", "forecast", "future", "next day", "next 24", "24 hour", "24h", "48 hour", "48h", "72 hour", "72h", "next three days", "next 3 days"))
        ...
        if any(term in text for term in ("cleanest", "best air", "healthiest air", "lowest aqi", "right now", "currently", "current")) and not future_requested:
            best_aqi, best_city = current_rank[0]
            return f"Among the supported cities, {best_city.title()} currently has the lowest AQI at {best_aqi:.1f}.\n\nLower AQI means cleaner air." + _stale_notice(evidence["comparison"])
        if future_requested:
            best_aqi, best_city = tomorrow_rank[0]
            return f"Among the supported cities, {best_city.title()} is forecast to have the cleanest air tomorrow at {best_aqi:.1f} AQI.\n\nThis comparison covers only the cities supported by this app." + _stale_notice(evidence["comparison"])
        return "Current AQI, cleaner to poorer:\n" + "\n".join(f"- {city.title()}: {aqi:.1f} AQI." for aqi, city in current_rank) + "\n\nLower AQI is better." + _stale_notice(evidence["comparison"])

    if not cities:
        if "guidance" in evidence:
            return str(evidence["guidance"])
        if "aqi_reference" in evidence:
            return str(evidence["aqi_reference"])
        return "Please name a supported city explicitly."
    ...
    if "history" in evidence:
        history_data = evidence["history"]
        requested_point = history_data.get("requested_point")
        if requested_point:
            offset = history_data.get("requested_offset_hours")
            period = "about one day ago" if offset == 24 else "about two days ago" if offset == 48 else "about three days ago" if offset == 72 else f"about {offset} hours ago"
            return f"{city.title()}'s AQI {period} was {requested_point['aqi']:.1f}.\n\nStored observation: {requested_point['time']}." + _stale_notice(history_data)
        points = history_data["points"]
        change = points[-1]["aqi"] - points[0]["aqi"]
        return f"{city.title()} changed {change:+.1f} AQI over the last {history_data['hours']} stored hours.\n\nIt moved from {points[0]['aqi']:.1f} to {points[-1]['aqi']:.1f}. Historical data ends at {points[-1]['time']}." + _stale_notice(history_data)
    ...
    eli5 = any(phrase in text for phrase in ("like i'm five", "like im five", "like i am five", "eli5", "five year old", "5 year old", "very simple", "super simple"))
    ...
    if current and not forecast:
        if eli5:
            return f"{city.title()}'s AQI is {current['aqi']:.1f}. That means the air is pretty dirty today. Think of AQI like a dirtiness score for the air: smaller is cleaner, bigger is dirtier. It is better not to run or play outside for a long time today." + _stale_notice(current)
        if asks_activity:
            return f"{city.title()}'s latest AQI is {current['aqi']:.1f} ({current['category']}).\n\n{_activity_advice(current['category'])}\n\nObservation: {current['observed_at_utc']}." + _stale_notice(current)
        return f"{city.title()}'s latest available AQI is {current['aqi']:.1f} ({current['category']}).\n\nObservation: {current['observed_at_utc']}." + _stale_notice(current)
    ...
    if forecast and not current:
        ...
        if any(term in text for term in ("72 hour", "72h")):
            item = next(item for item in forecasts if item["horizon_hours"] == 72)
            return f"{city.title()}'s 72-hour AQI forecast is {item['aqi']:.1f} ({item['category']}).\n\nForecast valid at: {item['valid_at_utc']}." + _stale_notice(forecast)
        return f"{city.title()}'s AQI forecast:\n\n{_forecast_summary(forecasts)}\n\nForecast issued from observation: {forecast['issued_at_utc']}." + _stale_notice(forecast)
...
def _deterministic_chat(message: str, requested_cities: list[str] | None = None, history: list[str] | None = None) -> dict[str, Any]:
    ...
    if ambiguous_city:
        return {**base, "answer": ambiguous_city}
    if _off_topic(lower) or not _has_aqi_intent(text, history):
        return {**base, "answer": "I am limited to supported AQI, weather, pollutant, forecast, history, and model-explanation questions for this project."}
    ...
    if len(cities) > 1:
        ...
        forecast_terms = ("forecast", "tomorrow", "future", "next 24", "24 hour", "24h", "48 hour", "48h", "72 hour", "72h", "next day", "next three days", "next 3 days")
        explicit_current_terms = ("current", "currently", "right now", "now", "today")
        generic_status_terms = ("aqi", "air", "air quality", "pollution", "smog", "looking")
        activity_terms = ("jog", "run", "walk", "exercise", "workout", "outside", "outdoors", "play", "kids", "children", "elderly", "commute", "commuting", "school", "picnic", "bike", "football")
        ...
        if wants_current:
            plan.append(("current", "get_current_aqi", (city,)))
        if wants_forecast:
            plan.append(("forecast", "get_aqi_forecast", (city,)))
        ...
        if wants_history:
            plan.append(("history", "get_aqi_history", (city, _history_hours(lower), _history_offset_hours(lower))))
...
def _execute_groq_route(correlation_id: str, route: dict[str, Any]) -> tuple[dict[str, Any], list[str], list[dict[str, Any]]]:
    ...
    city = cities[0]
    tool_plan = {
        "current_aqi": [("current", "get_current_aqi", (city,))],
        "forecast": [("forecast", "get_aqi_forecast", (city,))],
        "current_forecast": [("current", "get_current_aqi", (city,)), ("forecast", "get_aqi_forecast", (city,))],
        "history": [("history", "get_aqi_history", (city, route["history_hours"], route.get("history_offset_hours")))],
...
def chat(message: str, requested_cities: list[str] | None = None, history: list[str] | None = None) -> dict[str, Any]:
    ...
    if not settings.COPILOT_ENABLED:
        return {"answer": "AQI Copilot is currently disabled by configuration.", "tools_used": [], "tool_events": [], "evidence": {}, "provider": "disabled", "correlation_id": correlation_id, "generated_at_utc": generated_at}
    ...
    if not answer:
        ...
        answer = _answer(text, evidence, route.get("cities", []))
